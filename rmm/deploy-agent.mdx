...
- An enrollment token for that company — see [Enrollment tokens](#enrollment-tokens). Create one from the deploy page if the company does not have one yet.
- Administrator / `sudo` access on the target endpoint.

<Steps>
  <Step title="Open the deploy page">
    In NexusOS, go to **RMM Agents → Deploy Agent**. Every company in CRM is listed. A company that already has an enrollment token shows it; a company that does not shows **No enrollment token** with a **Create enrollment token** action.
  </Step>
  <Step title="Create the token, if the company has none">
    Click **Create enrollment token**. One token per company is enough — it is reusable for every endpoint you enroll for that client.
  </Step>
...
**Tokens are created only when you ask for one.** A company has no enrollment credential until someone clicks **Create enrollment token** on the deploy page. Opening the page does not create anything.
...
After a **Revoke**, the company returns to **No enrollment token** and stays that way — the token does not come back on its own. Create a new one when you next need to enroll an endpoint for that client.
